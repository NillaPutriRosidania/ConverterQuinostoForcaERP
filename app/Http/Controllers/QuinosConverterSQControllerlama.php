...
$rowPtr = [
    1 => 2, // start row (karena row 1 header)
    2 => 2,
    3 => 2,
];
...
foreach ($logicalRows as $row) {
    ...
    // Tentukan sheet berdasarkan invoiceFlags
    $flags = $invoiceFlags[$trxCode] ?? ['has_extra_or_ta' => false, 'has_discount' => false];

    if (!$flags['has_extra_or_ta']) {
        $targetSheetNo = 1; // tidak ada extra/takeaway
    } else {
        $targetSheetNo = $flags['has_discount'] ? 3 : 2; // ada extra/takeaway
    }
    ...
    $discountAmount = (float) str_replace(',', '', ($sum['discount'] ?? '0'));
    ...
    // Blank header kolom master kalau trx yang sama sudah pernah ditulis di sheet itu
...
// Simpan ke temp file lalu return download
